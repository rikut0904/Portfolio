...
func (r *ProductRepository) List(ctx context.Context) ([]product.Product, error) {
	var models []postgres.ProductModel
...
func (r *ProductRepository) Update(ctx context.Context, id string, input product.ProductPayload, _ time.Time) error {
	...
	result := r.store.DB.WithContext(ctx).Model(&postgres.ProductModel{}).Where("id = ?", id).Updates(updates)
	if result.Error != nil {
		return result.Error
	}
...
func (r *ProductRepository) Delete(ctx context.Context, id string) error {
	result := r.store.DB.WithContext(ctx).Delete(&postgres.ProductModel{}, "id = ?", id)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
...
func (r *ProductRepository) toDomain(m postgres.ProductModel) product.Product {
	var techs []string
