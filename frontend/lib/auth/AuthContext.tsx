...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logAuthEvent = useCallback(
    async (action: "login" | "logout", credential: string) => {
      try {
  ...
  const signIn = useCallback(
    async (username: string, password: string) => {
      ...
      const credential = basicAuthHeader(
        normalizedUsername,
        normalizedPassword,
      );
      const result = await fetchJSON<{ user: { uid: string; email: string } }>(
        "/api/auth/login",
        {
          method: "POST",
          headers: {
  ...
  const signOut = useCallback(async () => {
    try {
      if (session?.credential) await logAuthEvent("logout", session.credential);
    } finally {
      applySession(null);
    }
