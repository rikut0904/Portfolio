...
export function useProducts(filters: ProductFilters = {}) {
  ...
  const technologiesKey = (filters.technologies ?? []).join(",");
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (technologiesKey) {
        params.append("technologies", technologiesKey);
      }
      if (filters.status) params.append("status", filters.status);
      if (filters.deployStatus)
        params.append("deployStatus", filters.deployStatus);
      if (filters.createdYear)
        params.append("createdYear", filters.createdYear);
      if (filters.createdMonth)
        params.append("createdMonth", filters.createdMonth);
