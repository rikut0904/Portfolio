...
function ActivitiesContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isAddingNew) {
        ...
        // 編集
        const response = await fetch(`/api/activity-categories/${editingCategory.id}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (categoryId: string) => {
    if (!user) return;
    if (!confirm("このカテゴリを削除しますか？関連する課外活動も削除される可能性があります。")) return;
    ...
    <div className="min-h-screen bg-gray-100">
      <main className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
        <div className="mb-4 sm:mb-6">
          <Link href="/admin" className="text-blue-600 hover:text-blue-800 text-xs sm:text-sm mb-1 sm:mb-2 inline-block">
            ← ダッシュボード
          </Link>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
            <h1 className="text-lg sm:text-2xl lg:text-3xl font-bold text-gray-900">課外活動カテゴリ管理</h1>
        ...
        {isAddingNew && (
          <div className="bg-white rounded-lg shadow p-3 sm:p-6 mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">新規カテゴリ追加</h2>
