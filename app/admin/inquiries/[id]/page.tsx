      ...
      <main className="max-w-4xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-lg sm:text-2xl font-bold text-gray-900">お問い合わせ詳細</h1>
              <p className="text-sm text-gray-600">やり取りの履歴を確認できます</p>
            </div>
            <Link href="/admin/inquiries" className="text-sm text-blue-800 hover:text-gray-900">
              一覧へ戻る
            </Link>
          </div>
          ...
          {!loading && detail && (
            <>
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-4">
                ...
                <p className="text-sm text-gray-900">{detail.contactName || "-"}</p>
                <p className="text-xs text-gray-500">{detail.contactEmail || "-"}</p>
              </div>

              <div className="space-y-2">
                <p className="text-sm font-medium text-gray-700">対応状況を更新</p>
                  ...
                  {replies.map((reply) => (
                    <div key={reply.id} className="rounded-lg border border-gray-200 bg-white p-4">
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                        <span>{reply.senderType === "admin" ? "運営" : reply.senderName || "お問い合わせ者"}</span>
